...
class HelpMenu(commands.Cog):
    ...
    async def help(self, ctx):
        ...
        basic.add_field(name="!Jokes", value="Have JETbot tell a joke", inline=False)
        basic.add_field(name="!poll timer question ans emoji", value="Create a poll open for a short time with these options", inline=False)
        ...
        Games = discord.Embed(color=0x0080ff, type="rich", title="Games Commands", description="This is the commands to play Games")
        Games.add_field(name="!tictactoe @discordname", value="Starts a game of TicTacToe", inline=False)
        Games.add_field(name="!place spot(number)", value="Places the X or O in the requested spot if not taken", inline=False)
        Games.add_field(name="!stop", value="Stops the game of TicTacToe", inline=False)
        Games.add_field(name="!Hangman", value="Start a game of Hangman with JETbot", inline=False)        
        Games.add_field(name="!Guess", value="Guess a letter for Hangman", inline=False)
        Games.add_field(name="!End", value="Stops the game of Hangman", inline=False)
        
        embedlist.append(Games)
        ...
        Admin.add_field(name="!vccheck", value="Initiate the collection of server Voice Chat Stats", inline=False)
